...
import { VMConfig, VMStatusInfo } from '../types';
...
export default function VMEdit() {
  const [, params] = useRoute('/vm/:vmId/edit');
  const vmId = params?.vmId || '';
  const [, navigate] = useLocation();
  const { images, fetchImages, fetchVMs } = useStore();
  ...
  useEffect(() => {
    fetchImages();
    api.getQemuInfo().then((info) => {
      setQemuInfo(info);
      return Promise.all([
        api.getVM(vmId),
        api.getVMStatus(vmId),
      ]).then(([fetched, status]) => {
        setVm(fetched);
        setVmStatus(status);
        setName(fetched.name);
        setConfig({
          system: { ...fetched.system },
          storage: { ...fetched.storage },
          display: { ...fetched.display },
          network: { ...fetched.network },
        });
        setExtraArgs((fetched.extra_args || []).join(' '));

        const diskImg = fetched.storage.disk_image || '';
        if (!diskImg) {
          setDiskSource('auto');
        } else if (diskImg.startsWith(info.data_dir + '/images/') || diskImg.includes('/images/')) {
          setDiskSource('existing');
        } else {
          setDiskSource('external');
          setExternalPath(diskImg);
        }
        setLoading(false);
      });
    }).catch(() => {
  ...
  useEffect(() => {
    if (diskSource === 'existing' && vm && qemuInfo.data_dir) {
      const diskImg = vm.storage.disk_image || '';
      const match = images.find((i) => i.path === diskImg);
      if (match) {
        setExistingImage(match.name);
      }
    }
  }, [images, qemuInfo, vm, diskSource]);
  ...
  if (loading) {
    return <div className="text-center text-slate-400 py-12">Loading VM...</div>;
  }

  const isRunning = vmStatus?.status === 'running';
  const hasPendingChanges = vm && vm.config_generation > vm.running_generation;

  return (
    <div className="max-w-6xl mx-auto">
      <h2 className="text-2xl font-bold mb-6 text-white">Edit VM: {name}</h2>

      {isRunning && (
        <div className="bg-yellow-900/30 border border-yellow-700 rounded p-3 mb-4 text-yellow-200 text-sm">
          VM is running — changes will take effect after restart
        </div>
      )}

      {hasPendingChanges && (
        <div className="bg-amber-900/30 border border-amber-700 rounded p-3 mb-4 text-amber-200 text-sm">
          Changes pending — restart the VM to apply
        </div>
      )}
